...
### Keeper might not be able to invoke a tipped suggestion
...
### Tipper might be able to withdraw tips after the suggestion is invoked before tips are claimed
...
### Keepers can by pass platform fees with an unofficial Tip Jar
...
### Orb team can update `platformFee` to steal all tips
...
### Owner could stop a tipped suggestion being invoked 
...
### Duplicated invocations allowed by the Orb might not be suggestable into the tipping jar
...
### Lack of input checks on platformFee and platformAddress
...
### Unable to tip an edge case invocation
...
### Keeper could stop user tipping by front running it with `setMinimumTip`
...
### Tipping jar can be used by un-official orbs, causing pollution to event logs
...
### Anyone can claim tips for past invocations not registered in the tipping jar, causing pollution to the `TipsClaim` event logs
